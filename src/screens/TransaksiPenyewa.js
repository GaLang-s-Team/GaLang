...
export default function TransaksiPenyewa({ navigation, route }) {
  ...
  const fetchPermintaan = async () => {
    const permintaanRef = query(collection(firestore, 'informasi_penyewaan'), where('penyewa', '==', userId), where('status', '==', 'Menunggu Konfirmasi'));
    ...
    return (
      <View style={styles.card}>
        ...
        <TouchableOpacity style={{width:90, height:35, marginLeft:2.5, marginTop:65, justifyContent:'center', alignItems:'center', backgroundColor:'#459708', borderRadius:10}}
          onPress={() => navigation.navigate('UploadPembayaran', { userId: userId, transaksiId: transaksi.id_transaksi})}>
          <Text style={{color:'#FFFFFF', fontSize:16, fontWeight:'bold'}}>Bayar</Text>
        </TouchableOpacity>
      </View>
    );
  };
